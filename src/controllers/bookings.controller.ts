  ...
  static async getBookingInvoice(req: Request, res: Response) {
    try {
      ...
      const invoice = {
        ...
        service: (booking as any).service_name || booking.service.name,
        notes: booking.notes,
        staff: booking.staff?.display_name || '-',
        customer: booking.user?.profile?.full_name || (booking as any).customer_name || 'Walk-in',
        customerEmail: booking.user?.email || (booking as any).customer_email || '',
        customerPhone: booking.user?.profile?.phone || (booking as any).customer_phone || '',
        pdfUrl,
        salon: {
